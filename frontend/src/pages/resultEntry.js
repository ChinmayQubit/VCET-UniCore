...
export function initResultEntry() {
    ...
    if(resultForm) {
        resultForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await apiPost('/results', payload);
...
export async function initResultPage() {
    currentStudentId = getStudentId() || 1;
    document.getElementById('msStudentId').innerText = currentStudentId;

    try {
        const resultsRes = await apiFetch(`/results/student/${currentStudentId}`);
        ...
        window.renderSemesterMarksheet(latestSem);
...
function buildSemesterSelector(semesters) {
    ...
    semesters.forEach(sem => {
        ...
        btn.onclick = () => window.renderSemesterMarksheet(sem);
        selectorDiv.appendChild(btn);
    });
}

window.renderSemesterMarksheet = function(semester) {
    ...
    apiGet(`/results/cgpa/${currentStudentId}`)
        .then(cgpaNum => {
    ...
    apiGet(`/analysis/ai-cgpa/${currentStudentId}`)
        .then(data => {
    ...
    apiGet(`/analysis/target/${currentStudentId}`)
        .then(data => {
